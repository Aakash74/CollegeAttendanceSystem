	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for(AttendanceBean attendanceBean : attendanceBeanList)
		{
			...
			if(attendanceBean.getAttendance_session_id() != 0)
			{
				...
				users = attendanceBean.getAttendance_student_id()+".     "+studentBean.getStudent_firstname()+","+ ((StudentBean) studentBean).getStudent_lastname()+"                  "+attendanceBean.getAttendance_status();
			}
			else
			{
